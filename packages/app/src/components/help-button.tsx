    ...
    <Drawer open={drawerOpen()} onOpenChange={setDrawerOpen} side="right">
      <Show when={settings.general.shouldDisplayTabsToast()}>
        <div
          ...
          aria-label={language.t("help.tabs.toast.aria")}
        >
          <button
            type="button"
          ...
          <button
            type="button"
            class="relative block h-[232px] w-[184px] cursor-pointer overflow-hidden rounded-[4px] bg-[#111] text-left"
            onClick={() => {
          ...
          >
            <div
              class="absolute inset-0 flex items-center justify-center bg-[radial-gradient(circle_at_50%_35%,#343434_0%,#111_65%)]"
              aria-hidden="true"
            >
              <span class="font-mono text-[20px] font-semibold tracking-[-0.5px] text-white">KoteCode</span>
            </div>
      ...
      >
        ...
        <div class="relative flex min-h-0 w-full flex-1 flex-col items-start gap-6 overflow-y-auto p-8">
          ...
          <div class="flex w-full flex-1 flex-col gap-4 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-base">
            <p>{language.t("help.tabs.body.intro")}</p>
            <p>{language.t("help.tabs.body.start")}</p>
            <p>{language.t("help.tabs.body.organize")}</p>
            <p>{language.t("help.tabs.body.home")}</p>
            <p>{language.t("help.tabs.body.persist")}</p>
            <p>{language.t("help.tabs.body.worktrees")}</p>
          </div>
        </div>
      </DrawerContent>
    </Drawer>
