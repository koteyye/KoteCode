...
function globalConfigFile() {
  const candidates = ["kotecode.jsonc", "kotecode.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "kotecode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "kotecode.jsonc"), env))
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR || dir === Flag.KOTECODE_CONFIG_DIR) {
            for (const file of ["opencode.json", "opencode.jsonc", "kotecode.json", "kotecode.jsonc"]) {
